...
**ecbot:** exact values and ecbot's stack (motion import path, `ui-` tokens, endorsed libraries) are pinned in [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) — STANDARDS.md mirrors those values. Findings must cite ecbot easing tokens, `motion/react`, and `ui-` color tokens; flag any `framer-motion`/`sonner`/hardcoded-hex/`ease-in`-on-UI usage as a violation of ecbot's design system, not just craft.
