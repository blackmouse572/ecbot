...
**ecbot:** these values are pinned once in [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) — express every finding in ecbot's terms (easing tokens `--ease-out`/`--ease-in-out`/`--ease-drawer`, `motion/react`, `ui-` color tokens, `.dark` variant). `framer-motion` imports, `sonner`, hardcoded hex, and `ease-in` on UI are ecbot design-system violations.
