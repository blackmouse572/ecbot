...
**ecbot:** exact motion values (easing tokens, durations, spring configs) and library/token choices come from the single source of truth — [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) and [`emil-design-eng/SKILL.md`](../emil-design-eng/SKILL.md). Every recipe you propose must use ecbot's easing tokens, `motion/react`, and `ui-` color tokens — don't approximate curves or invent parallel ones.
...
Every "Suggested motion" cell carries exact values — the curve, the duration, the properties — pulled from ecbot's shared vocabulary in [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) (`--ease-out`, `--ease-in-out`, `--ease-drawer`; add them to `apps/app/src/index.css @theme` if a suggestion needs one that isn't defined yet), never approximated. Animate `transform` and `opacity` only; include reduced-motion handling (gentler, not zero) and `@media (hover: hover) and (pointer: fine)` gating when the suggestion involves hover.
