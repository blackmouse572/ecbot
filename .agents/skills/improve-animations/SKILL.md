...
The audit lens (categories, severity) lives in [AUDIT.md](AUDIT.md); the plan format in [PLAN-TEMPLATE.md](PLAN-TEMPLATE.md). Load them when you audit and when you write plans.

**ecbot:** exact motion values (easing curves, durations, spring configs) and ecbot's stack (motion library import, `ui-` tokens, endorsed libraries) come from the single source of truth — [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) and [`emil-design-eng/SKILL.md`](../emil-design-eng/SKILL.md). AUDIT.md tells you _what to look for_; pull the _exact target value_ from there. Every plan you write must specify ecbot tokens, `motion/react` imports, and `ui-` color tokens — never a raw hardcoded curve or `framer-motion`.
