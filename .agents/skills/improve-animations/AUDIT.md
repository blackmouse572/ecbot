...
The eight audit categories and what to look for in each. Distilled from Emil Kowalski's design engineering philosophy ([emilkowal.ski](https://emilkowal.ski/)).

**ecbot — where values come from:** the curves, durations, spring configs, and library/token choices are pinned once in [`emil-design-eng/eccho.md`](../emil-design-eng/eccho.md) (with deeper rationale in [`emil-design-eng/SKILL.md`](../emil-design-eng/SKILL.md)). Use this playbook as the _audit lens_ and cite the _exact value_ from there — express fixes as ecbot easing tokens (`--ease-out`/`--ease-in-out`/`--ease-drawer`), `motion/react` imports, and `ui-` color tokens. Never approximate a curve or reach for `framer-motion`.
