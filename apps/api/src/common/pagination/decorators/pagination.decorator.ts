...
import { PaginationFilterContainInArrayPipe } from 'src/common/pagination/pipes/pagination.filter-contain-in-array.pipe';
...
export function PaginationQueryFilterContainInArray(
    field: string,
    defaultValue?: string[],
    options?: IPaginationFilterOptions
): ParameterDecorator {
    return Query(
        options?.queryField ?? field,
        PaginationFilterContainInArrayPipe(field, defaultValue, options)
    );
}
