...
@Injectable()
export class ResponseFileExcelInterceptor implements NestInterceptor<
    Promise<any>
> {
    constructor(
            ...
            return next.handle().pipe(
                map(async (res: Promise<IResponseFileExcel>) => {
                    ...
                    if (type === ENUM_HELPER_FILE_EXCEL_TYPE.XLSX) {
                        // create file
                        const file: Buffer = await this.fileService.writeExcel(
                            responseData.data
                        );
                    ...
                    // create file
                    const file: Buffer = await this.fileService.writeCsv(
                        responseData.data[0]
                    );
