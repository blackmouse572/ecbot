...
import { AwsS3PresignDto } from '../dtos/aws.s3-presign.dto';
...
export class AwsS3Service implements OnModuleInit, IAwsS3Service {
    ...
    onModuleInit(): void {
        // A custom endpoint (e.g. Cloudflare R2, MinIO) needs path-style
        // addressing; without it the AWS SDK builds AWS virtual-host URLs.
        ...
        this.config.private.client = new S3Client({
            ...
            ...(this.config.private.endpoint
                ? {
                      endpoint: this.config.private.endpoint,
                      forcePathStyle: true,
                  }
                : {}),
        });
    ...
    getFileInfo(key: string): IAwsS3FileInfo {
        ...
        // Resolve mime by exact extension first (handles md/txt/docx/epub where
        // the mime string doesn't end with the extension), then fall back to the
        // suffix heuristic, then a safe default so mime is never empty.
        const ext = extension.toLowerCase();
        const mime: string =
            (Object.entries(EXTENSION_BY_MIME).find(
                ([, e]) => e === ext
            )?.[0] as string) ||
            Object.values(ENUM_FILE_MIME).find(e =>
    ...
    /**
     * Downloads an S3 object and returns its raw bytes as a Node.js Buffer.
     * Uses the AWS SDK v3 idiomatic stream-to-bytes conversion.
     * @param {string} key - The S3 object key (must not start with "/")
     * @param {IAwsS3Options} [options] - Optional access level (defaults to public)
     * @returns {Promise<Buffer>} Raw bytes of the object
     */
    async getItemBuffer(key: string, options?: IAwsS3Options): Promise<Buffer> {
        if (key.startsWith('/')) {
            throw new Error('Key should not start with "/"');
        }
        const config = this.getConfig(options);
        ...
        const output: GetObjectCommandOutput = await config.client.send<
            GetObjectCommandInput,
            GetObjectCommandOutput
        >(command);
        if (!output.Body) {
        ...
        const bytes = await (output.Body as any).transformToByteArray();
    ...
    async putItem(
        ...
        const isPrivate = options?.access === ENUM_AWS_S3_ACCESSIBILITY.PRIVATE;
