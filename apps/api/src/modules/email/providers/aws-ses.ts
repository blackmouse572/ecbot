...
export class AwsSESProvider implements IEmailService {
    ...
    async sendInvitationToWorkSpace(
        ...
        try {
            await this.awsSESService.send({
                templateName: ENUM_SEND_EMAIL_PROCESS.INVITE_TO_WORKSPACE,
                recipients: [email],
                sender: this.fromEmail,
                templateData: {
                    homeName: this.homeName,
    ...
    async sendAccountBlocked(
        ...
        try {
            await this.awsSESService.send({
                templateName: ENUM_SEND_EMAIL_PROCESS.ACCOUNT_BLOCKED,
                recipients: [email],
                sender: this.fromEmail,
                templateData: {
                    homeName: this.homeName,
    ...
    async sendAccountBanned({ name, email }: EmailSendDto): Promise<boolean> {
        try {
            await this.awsSESService.send({
                templateName: ENUM_SEND_EMAIL_PROCESS.ACCOUNT_BANNED,
                recipients: [email],
                sender: this.fromEmail,
                templateData: {
                    homeName: this.homeName,
    ...
    async sendLowTokenBalance(
        ...
        try {
            await this.awsSESService.send({
                templateName: ENUM_SEND_EMAIL_PROCESS.LOW_TOKEN_BALANCE,
                recipients: [email],
                sender: this.fromEmail,
                templateData: {
                    homeName: this.homeName,
                    ...
                    usedPercent,
                    periodEnd:
                        this.helperDateService.formatToRFC2822(periodEnd),
