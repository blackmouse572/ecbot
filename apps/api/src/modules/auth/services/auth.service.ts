...
export class AuthService implements IAuthService {
    ...
    constructor(
        ...
        this.jwtAccessTokenPrivateKey = readAuthFile(
            this.configService.get<string>(
                'auth.jwt.accessToken.privateKeyPath'
            )
        );
        this.jwtAccessTokenPublicKey = readAuthFile(
            this.configService.get<string>('auth.jwt.accessToken.publicKeyPath')
        );
        ...
        this.jwtRefreshTokenPrivateKey = readAuthFile(
            this.configService.get<string>(
                'auth.jwt.refreshToken.privateKeyPath'
            )
        );
        this.jwtRefreshTokenPublicKey = readAuthFile(
            this.configService.get<string>(
    ...
    // Verifies the refresh token's signature (so a forged/tampered cookie is
    // rejected) while tolerating expiry, so logout can still identify and
    // revoke the session behind an already-expired refresh cookie.
    ...
    createToken(
        ...
        const payloadRefreshToken: IAuthJwtRefreshTokenPayload =
            this.createPayloadRefreshToken(payloadAccessToken, rememberMe);
    ...
    // Mirrors setRefreshTokenCookie's options exactly (same sameSite helper,
    // same buildRefreshCookieOptions builder) — a mismatch here and the
    // browser silently keeps the cookie instead of dropping it.
    ...
    private getRefreshCookieSameSite(): CookieOptions['sameSite'] {
        ...
        return env === ENUM_APP_ENVIRONMENT.LOCAL ? 'none' : 'strict';
    }
}
