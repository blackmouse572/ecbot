...
- **@PaginationQueryFilterDate(field, options?)**: Filter dates with comparison operators ($gte, $lte, $gt, $lt)
    - `field`: Database field name
    - `options.queryField`: Custom query parameter name (defaults to field name)
    - Accepts URL-encoded JSON with date comparison operators (e.g., `{"$gte":"2025-09-23T17:00:00.000Z","$lte":"2025-09-27T17:00:00.000Z"}`)
...
- **PaginationFilterDatePipe**: Creates single field date queries with flexible comparison operators (`$gte`, `$lte`, `$gt`, `$lt`)
...
> **Bare `@PaginationQuery()` carries no filters.** It resolves `search`, `page`,
> `perPage`, `orderBy` and `orderDirection` — nothing else. Any other query param on a
> list endpoint needs its own `@PaginationQueryFilter*` decorator (see
> [Advanced Filtering](#advanced-filtering)); `search` only works once you pass
> `availableSearch`. Accepting a filter with a raw `@Query('foo')` skips validation and
> value coercion, silently disables the search whitelist, and forces an inline
> `@ApiQuery` instead of `DocRequest({ queries })` — three conventions broken at once.
...
async list(
    ...
    @PaginationQueryFilterDate('createdAt') createdAtFilter: Record<string, any>
) {
...
### Single Field Date Filter

The new `@PaginationQueryFilterDate` decorator supports flexible date filtering with comparison operators:

```typescript
// Controller method with new date filter
async findAll(
    @PaginationQuery() paginationQuery: Record<string, any>,
    @PaginationQueryFilterDate('createdAt') createdAtFilter: Record<string, any>
) {
    // createdAtFilter will contain MongoDB query like: { createdAt: { $gte: Date, $lte: Date } }
}
```
...
```
GET /api/users?createdAt={"$gte":"2025-09-02T00:00:00.000Z","$lte":"2025-09-02T23:59:59.999Z"}
```

**Last 7 days:**

```
GET /api/users?createdAt={"$gte":"2025-08-26T00:00:00.000Z"}
```

**Before specific date:**

```
GET /api/users?createdAt={"$lt":"2025-09-01T00:00:00.000Z"}
```

**URL Encoded (as used by frontend):**

```
GET /api/users?createdAt=%7B"%24gte"%3A"2025-09-23T17%3A00%3A00.000Z"%2C"%24lte"%3A"2025-09-27T17%3A00%3A00.000Z"%7D
```
