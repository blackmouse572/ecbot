...
Service should contain a `mapDetail` method that maps the response data to the standard format
...
Service should contain a `mapList` method that maps the response data to the standard format

```typescript
  mapList(skills: SkillEntity[]): SkillListResponseDto[] {
      return plainToInstance(SkillListResponseDto, skills, {
          excludeExtraneousValues: true,
      });
  }
```

Note: We use `plainToInstance` to map the response data to the standard format. and `excludeExtraneousValues: true` to exclude any fields that are not defined in the DTO. So, in dto you need to add `@Expose()` decorator to the fields you want to include in the response.
