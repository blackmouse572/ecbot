...
- [Table of Contents](#table-of-contents)
    - [Standard Installation](#standard-installation)
        ...
        - [Infrustructure Setup](#infrastructure-setup)
...
4. [pnpm](https://pnpmpkg.com) (v1.22.x)
...
We already have infrustructre as code with `terraform` and `docker-compose`, so you can easily set up the required infrastructure on your local machine or in the cloud.
...
Then, navigate to the `infrastructure` directory and run the following commands to set up the infrastructure:
...
Review the plan output to ensure it matches your expectations, then apply the changes:
...
```bash
pnpm migrate:template
```

To roll back template changes:

```bash
pnpm rollback:template
```
...
docker-compose exec apis pnpm migrate:seed

# For a complete reset and rebuild
docker-compose exec apis pnpm migrate:fresh

# To roll back migrations
docker-compose exec apis pnpm migrate:remove
```
