...
interface DataTableProps<TData> {
  data?: TData[];
  // `any` replaces the apps' ambient `type A = any`, which is not available
  // inside this package (same rationale as DataTableRootProps["columns"]).
...
export const DataTable = <TData,>({
  ...
  const effectiveEnableColumnVisibility = enableColumnVisibility;
  ...
  const onRowClick = useCallback(
    (event: React.MouseEvent<HTMLTableRowElement, MouseEvent>, row: TData) => {
      ...
      // This package has no `vite/client` type reference (it isn't a Vite app
      // itself, just consumed by one), so `import.meta.env` isn't ambiently
      // typed here the way it is in apps/app's vite-env.d.ts.
      const basePath =
