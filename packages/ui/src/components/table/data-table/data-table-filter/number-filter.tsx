...
export const NumberFilter = ({
  ...
  const debouncedOnChange = useCallback(
    debounce((e: ChangeEvent<HTMLInputElement>, operator: Operator) => {
      ...
      let curr: unknown;
      try {
        curr = JSON.parse(currentValue?.join(",") || "{}");
      } catch {
        curr = {};
      }
      ...
      const handleValue = (operator: Operator) => {
        ...
        if (curr && !value) {
          // Narrowing for `strict`, which this package compiles under and the
          // app it moved from did not. `curr` is the operator-object form here;
          // the bare-number form already returned above.
          if (typeof curr === "object") {
            delete (curr as Record<string, unknown>)[operator];
          }
...
const parseValue = (value: string[] | null | undefined) => {
  ...
  // `val` is raw URL input — never let a malformed value throw during render.
  try {
