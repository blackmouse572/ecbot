...
export const DateFilter = ({
  ...
  const handleCustomDateChange = (value: Date | null, pos: "start" | "end") => {
    ...
    // DatePicker fires onChange once on mount with its own `value` prop, which
    // still reflects the prior render's comparison (e.g. the preset just
    // cleared by handleSelectCustom) — writing it back would silently resurrect
    // whatever this control was just cleared of. Only a value the user actually
    // picked, which differs from what's already stored, should write through.
    if ((currentDateComparison?.[key] ?? undefined) === dateValue) {
      return;
    }
...
const usePresets = () => {
  ...
  return useMemo(() => {
    // Anchor computed per render (not module scope) so presets don't go stale
    // at midnight in a long-lived admin tab.
    ...
    return [
      { label: t("filters.date.today"), value: { $gte: today.toISOString() } },
      {
...
const parseDateComparison = (value: string[]) => {
  ...
  // `value` comes from the URL query string — a crafted `?…=not-json` must
  // degrade to an empty filter, not throw during render.
  try {
    const parsed = JSON.parse(value.join(","));
    return typeof parsed === "object" && parsed !== null
      ? (parsed as DateComparisonOperator)
      : null;
  } catch {
    return null;
  }
