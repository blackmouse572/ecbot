...
export interface DataTableRootProps<TData> {
   ...
   *
   * `any` for the value type is deliberate and matches TanStack's own typing:
   * a `createColumnHelper` array mixes accessor columns (TValue = the field
   * type) with display columns (TValue = unknown), and that union is not
   * assignable to a single `ColumnDef<TData, unknown>[]`. This replaces the
   * apps' ambient `type A = any`, which is not available inside this package.
   */
              ...
              {table.getRowModel().rows.map((row) => {
                ...
                return (
                  <Table.Row
                    ...
                    className={clx(
                      "min-h-12 transition-fg group/row group relative [&_td:last-of-type]:w-[1%] [&_td:last-of-type]:whitespace-nowrap",
                  ...
                  >
                    {cells.map((cell, index) => {
                      ...
                      const isFirstCell =
                        firstCell !== -1
                          ? cell.column.id ===
                            visibleCells[firstCell]?.column.id
                          : index === 0;
